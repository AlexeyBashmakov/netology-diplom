    ...
    for i in range(1, len(table)):
        try:
            ...
            if table[i]["CLIENT_COD"].strip().isnumeric():
                ...
                # пропускаю также строки где не указан пол пациента (стоит пробел) и день рождения
...
def priems_to_csv():
    ...
    # следовательно, если для пациента на данный день присутствует ненулевой платеж, то считаем прием платным
...
# причина такой разницы думаю в том, что, в первом случае, ноутбук работал от сети и использовал максимум ресурсов
# а, во втором, ноутбук работал от аккумулятора и для сохранения заряда оного процессор работал на минимальной скорости
union_datas()
if True:
